...
void WikiDialog::setSelectedPageReadStatus(bool read)
{
	...
	RsThread::async([self, msgId, groupId, pageId, read]()
	{
		...
		if (!rsWiki->setMessageReadStatus(msgId, read))
		{
#ifdef WIKI_DEBUG
...
void WikiDialog::loadPages(const RsGxsGroupId &groupId)
{
	...
	RsThread::async([this, groupId]()
	{
		...
		if (!rsWiki->getSnapshots(
		        std::list<RsGxsGroupId>({groupId}), snapshots,
		        RS_TOKREQOPT_MSG_LATEST | RS_TOKREQOPT_MSG_THREAD))
		{
...
void WikiDialog::loadWikiPage(const RsGxsGrpMsgIdPair &msgId)
{
	...
	{
		std::vector<RsWikiSnapshot> snapshots;
		if (!rsWiki->getSnapshots(msgId, snapshots) || snapshots.empty())
		{
